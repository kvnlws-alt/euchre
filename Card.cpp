...
//EFFECTS returns the next suit, which is the suit of the same color
  Suit Suit_next(Suit suit) {
    return string_to_suit(SUIT_NAMES[(suit + 2) % 4]);
  }
...
bool Card_less(const Card &a, const Card &b, Suit trump) {
  ...
  if (suitA == suitB) {
    Rank aRank = a.get_rank();
    Rank bRank = b.get_rank();
    if (aRank < bRank) {
      return true;
    } else {
      return false;
...
bool Card_less(const Card &a, const Card &b, const Card &led_card, Suit trump) {
  Suit suitA = a.get_suit(trump);
  Suit suitB = b.get_suit(trump);
  Suit suitLed = led_card.get_suit(trump);
...
*/
  ...
  bool a_led = (suitA == suitLed);
  bool b_led = (suitB == suitLed);
